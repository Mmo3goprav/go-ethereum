...
func unknownTx(s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce()+1, to, tx.Value(), tx.Gas(), tx.GasPrice(), tx.Data(), tx.Author())
...
func invalidNonceTx(s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce()-2, to, tx.Value(), tx.Gas(), tx.GasPrice(), tx.Data(), tx.Author())
	return signWithFaucet(s.chain.chainConfig, txNew)
}
...
func hugeAmount(s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce(), to, amount, tx.Gas(), tx.GasPrice(), tx.Data(), tx.Author())
	return signWithFaucet(s.chain.chainConfig, txNew)
}
...
func hugeGasPrice(s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce(), to, tx.Value(), tx.Gas(), gasPrice, tx.Data(), tx.Author())
	return signWithFaucet(s.chain.chainConfig, txNew)
}
...
func hugeData(s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce(), to, tx.Value(), tx.Gas(), tx.GasPrice(), largeBuffer(2), tx.Author())
	return signWithFaucet(s.chain.chainConfig, txNew)
}
